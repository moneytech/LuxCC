...
void analyze_static_initializer(ExecNode *e, int is_addr)
{
    ...
    case OpExp:
        ...
        case TOK_DOT:
            if (!is_addr)
                goto bad_init;
            analyze_static_initializer(e->child[0], is_addr);
            break;

        case TOK_SIZEOF:
            break;
        case TOK_ADDRESS_OF:
            analyze_static_initializer(e->child[0], TRUE);
            break;
        case TOK_ARROW:
        case TOK_INDIRECTION:
            goto bad_init;
        ...
        case TOK_NEGATION:
        case TOK_CAST:
            analyze_static_initializer(e->child[0], FALSE);
            break;
        ...
        case TOK_BW_XOR:
        case TOK_BW_OR:
        case TOK_AND: /* TOFIX: the right operand should only be analyzed if the left is true. */
        case TOK_OR:  /* TOFIX: the right operand should only be analyzed if the left is false. */
            analyze_static_initializer(e->child[0], FALSE);
            analyze_static_initializer(e->child[1], FALSE);
            break;
        case TOK_CONDITIONAL: /* TOFIX: similar to &&/|| */
            analyze_static_initializer(e->child[0], FALSE);
            analyze_static_initializer(e->child[1], FALSE);
            analyze_static_initializer(e->child[2], FALSE);
            break;
        }
        break;
    case IConstExp:
    ...
    return; /* OK */
bad_init:
    FATAL_ERROR(e, "invalid initializer for static object");
}
