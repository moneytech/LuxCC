...
main(int argc, char **argv)
{
	...
	while( (i<argc) && (argv[i][0] == '-')){
		  ...
		  if(strcmp(argv[i],"-r")==0){
			  ...
			  printf("\ta(x,0)     = x + 1\n");
			  printf("\ta(0,y+1)   = a(1,y)\n");
			  printf("\ta(x+1,y+1) = a(a(x,y+1),y)\n\n");
			  printf("where x and y are nonnegative integers.\n");
			  printf("It is known that a is recursive, but not primitive recursive.\n\n");
			  exit(0);
	          }
		  fprintf(stderr, "ackermann: Unknown option %s\n", argv[i]);
		  fprintf(stderr, "%s\n",USAGE);
		  return 1;
	}
